...
### ModInt 不使用
```rs
let cl = convolution_raw::<i64, Mod998244353>(&al, &bl);
```
それぞれの型は `Vec<i64>`

### ModInt 使用
```rs
let cml = convolution::<Mod998244353>(&aml, &bml);
```
それぞれの型は `Vec<StaticModInt<Mod998244353>>`
@doc.text.end */

// @doc.subtitle {例題}
// @doc.text.inline [ACL-F](https://atcoder.jp/contests/practice2/tasks/practice2_f): 畳み込みの結果を Mod で求めるだけ
// @doc.src.begin
fn solve1() {
    input! {
        n: usize, m: usize,
...
fn solve2() {
    ...
    // ModInt 使用
    let aml = al.iter().map(|&x| ModInt998244353::new(x)).collect_vec();
    let bml = bl.iter().map(|&x| ModInt998244353::new(x)).collect_vec();
    let cml = convolution::<Mod998244353>(&aml, &bml);
    for c in cml {
        print!("{} ", c);
    }
